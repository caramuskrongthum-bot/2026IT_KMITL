...
sampler2D _HairWindMap;
half4 _HairWindMap_ST;
half _HairWindSpeed;
half _HairWindFrequency;
half _HairWindStrength;
half3 _HairWindDirection;
...
// FUNCTIONS
//////////////////////////////////////////////////////////////////////////////////////////
float3 HairWind(appdata_full v) {
	float3 pos = v.vertex.xyz;
	#if HAIR_WIND
		// Sample in the vertex shader with an explicit LOD (tex2D has no derivatives here)
		half windMask = tex2Dlod(_HairWindMap, float4(TRANSFORM_TEX(v.texcoord.xy, _HairWindMap), 0, 0)).r;

		// Black = move, White = still
		half windAmount = (1 - windMask) * _HairWindStrength;

		// Semi-random per-object phase so multiple hair objects don't move in lockstep
		float3 worldPosCenter = TransformObjectToWorld(float3(0, 0, 0)).xyz;
		half phase = dot(worldPosCenter, 1);

		half wave = sin(_Time.y * _HairWindSpeed * _HairWindFrequency + phase);

		float3 windDir = normalize(_HairWindDirection);
		pos += windDir * wave * windAmount;
	#endif
	return pos;
}
