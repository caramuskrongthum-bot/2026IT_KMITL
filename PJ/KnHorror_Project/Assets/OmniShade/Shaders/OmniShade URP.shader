...
Shader "OmniShade/Standard URP" {
	Properties {
		...
		[HeaderGroup(Hair Wind Dynamics)]
		[Toggle(HAIR_WIND)] _HairWind ("Enable Hair Wind", Float) = 0
		_HairWindMap ("Wind Map (Black = Move, White = Still)", 2D) = "white" {}
		_HairWindSpeed ("Wind Speed", Range(0, 10)) = 3.0
		_HairWindFrequency ("Wave Frequency", Range(0, 10)) = 2.0
		_HairWindStrength ("Wind Strength", Range(0, 1)) = 0.15
		_HairWindDirection ("Direction (XYZ)", Vector) = (1, 0.2, 0, 0)
	...
	Subshader {
		...
		Pass {
			...
			#endif
			#pragma shader_feature HAIR_WIND
